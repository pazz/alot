...
def generate_command_tables():
    modes = []
    base = os.path.join(HERE, 'usage', 'modes')
    os.makedirs(base)
    for mode, modecommands in COMMANDS.iteritems():
        modefilename = mode + '.rst'
        with open(os.path.join(base, modefilename), 'w') as modefile:
            modefile.write(NOTE)
            if mode != 'global':
                modes.append(mode)
                header = 'Commands in `{}` mode'.format(mode)
            ...
            for cmdstring, struct in modecommands.iteritems():
                parser = struct[1]
                labelline = '.. _cmd.{}.{}:\n\n'.format(
